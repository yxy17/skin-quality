...
def list_to_txt(list1,out_file):
    ## output form [x_leftop, y_lefttop, x_rightbottm, y_rightbottom,(x0,y0,x1,y1)
    ##input form(y0,x0,y1,x1)
    f_o = open(out_file, 'w')
    for item in list1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--thresh', dest='thresh', default=35, type=float, help='threshold for edge detection')
    parser.add_argument('-n', '--count_num', dest='count_num', default=None, type=int, help='The number of test images')
    parser.add_argument('--save_path', type=str, default='./bbox_output', help="paths for saving the final output bbox")
    parser.add_argument('--mask_save_path', type=str, default='./mask', help="paths for saving skin mask")
    parser.add_argument('--edge_save_path', type=str, default='./edge', help="paths for saving image edge ")
